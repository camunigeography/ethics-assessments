...
class ethicsAssessments extends reviewableAssessments
{
	...
	public function databaseStructureSpecificFields ()
	{
		...
		return $sql = "
			  ...
			  `funder` varchar(255) DEFAULT NULL COMMENT 'Funder',
			  `title` varchar(255) DEFAULT NULL COMMENT 'Project title',
			  `topic` text COMMENT 'Summary of topic',
			  `guidance` enum('','Yes','No') DEFAULT NULL COMMENT 'Read guidance',
			  `methods` text COMMENT 'Data collection methods',
			  `conflicts` enum('','Yes','No') DEFAULT NULL COMMENT 'Conflicts of interest',
			  `conflictsDetails` text COMMENT 'Conflicts of interest - details',
			  `socialMedia` enum('','Yes','No') DEFAULT NULL COMMENT 'Social media',
			  `socialMediaDetails` text COMMENT 'Social media - details',
			  `harm` enum('','Yes','No') DEFAULT NULL COMMENT 'Harmful/intrusive',
			  `harmDetails` text COMMENT 'Harmful/intrusive - details',
			  `gatekeeper` enum('','Yes','No') DEFAULT NULL COMMENT 'Gatekeeper',
			  `gatekeeperDetails` text COMMENT 'Gatekeeper - details',
			  `organisations` enum('','Yes','No') DEFAULT NULL COMMENT 'Organisations',
			  `organisationsDetails` text COMMENT 'Organisations - details',
			  `vulnerable` enum('','Yes','No') DEFAULT NULL COMMENT 'Vulnerable participants',
			  `vulnerableDetails` text COMMENT 'Vulnerable participants - details',
			  `sensitiveTopics` enum('','Yes','No') DEFAULT NULL COMMENT 'Sensitive topics',
			  `sensitiveTopicsDetails` text COMMENT 'Sensitive topics - details',
			  `humanParticipants` enum('','Yes','No') DEFAULT NULL COMMENT 'Human participants',
			  `humanParticipantsDetails` text COMMENT 'Human participants - details',
			  `humanData` enum('','Yes','No') DEFAULT NULL COMMENT 'Data from humans',
			  `humanDataDetails` text COMMENT 'Data from humans - details',
			  `covert` enum('','Yes','No') DEFAULT NULL COMMENT 'Covert research',
			  `covertDetails` text COMMENT 'Covert research - details',
			  `inducements` enum('','Yes','No') DEFAULT NULL COMMENT 'Inducements',
			  `inducementsDetails` text COMMENT 'Inducements - details',
			  `assistants` enum('','Yes','No') DEFAULT NULL COMMENT 'Assistants',
			  `assistantsDetails` text COMMENT 'Assistants - details',
			  `distress` ENUM('','Yes','No') DEFAULT NULL COMMENT 'Distress',
			  `distressDetails` TEXT DEFAULT NULL COMMENT 'Distress - details',
			  `abroad` enum('','Yes','No') DEFAULT NULL COMMENT 'Abroad',
			  `abroadDetails` text COMMENT 'Abroad - details',
			  `dataProtection` text COMMENT 'Data protection',
